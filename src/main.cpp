...
const string strMessageMagic = "WaveCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 <= 1500)
    {
      nSubsidy = 50 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 15000)
    {
      nSubsidy = 30* COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 35000)
    {
      nSubsidy = 20 * COIN;
      return nSubsidy + nFees;
    }

    else if (pindexBest->nHeight+1 <= 46500)
    {
        nSubsidy = 10 * COIN;
        return nSubsidy + nFees;
    }
...
const int DAILY_BLOCKCOUNT =  5000;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "WaveCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Wavecoin - Saturday 280614";
        CTransaction txNew;
        txNew.nTime = 1402724989;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402724989;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1788914;
		if(fTestNet)
        {
            block.nNonce   = 1788914;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xc5e3fa38e51f5d106f72a47d5ca3878bb65e512e46d9d1a195b04bf53cfe6926"));
        block.print();
